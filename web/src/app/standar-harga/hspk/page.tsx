...
import { Search, Filter, Settings, Edit, Trash2, Plus, Loader2, X, Archive, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Trash } from 'lucide-react';
import { supabase } from '@/lib/supabaseClient';
import SpreadsheetUpload from '@/components/SpreadsheetUpload';

export default function HspkPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (searchTerm.length > 2) {
                query = query.or(`uraian.ilike.%${searchTerm}%,kode.ilike.%${searchTerm}%`);
            }

            const { data: hspk, count, error } = await query
                .order('kode', { ascending: true })
                .range(from, to);

            if (error) throw error;

            if (hspk) setData(hspk);
    ...
    const handleUpload = async (rows: any[]) => {
        ...
        try {
            ...
            for (let i = 0; i < mappedData.length; i += chunkSize) {
                ...
                const { error } = await supabase.from('hspk').insert(chunk);
                if (error) throw error;
            }
    ...
    const handleClearData = async () => {
        if (!confirm('PERINGATAN: Apakah Anda yakin ingin menghapus SEMUA data HSPK? Tindakan ini tidak dapat dibatalkan.')) return;

        setIsLoading(true);
        try {
            const { error: deleteError } = await supabase.from('hspk').delete().gt('id', 0);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const { error } = await supabase.from('hspk').delete().eq('id', id);
            if (error) throw error;
            fetchData();
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-navy-900 tracking-tight">Harga Satuan Pokok Kegiatan (HSPK)</h1>
                    <p className="text-gray-500 text-sm">Referensi Harga Satuan Pokok Kegiatan</p>
                </div>
            ...
            <Card>
                <CardHeader className="py-4 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
                    <CardTitle className="text-base font-medium">Data HSPK <span className="text-xs text-gray-400 font-normal ml-2">(Total: {totalItems})</span></CardTitle>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden">
                        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="text-lg font-bold text-gray-900">{isEditing ? 'Edit Item HSPK' : 'Tambah Item HSPK'}</h3>
