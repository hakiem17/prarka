...
export default function SbuPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const { data: sbu, count, error } = await query
    ...
    const handleUpload = async (rows: any[]) => {
        ...
        try {
            ...
            for (let i = 0; i < mappedData.length; i += chunkSize) {
                ...
                const { error } = await supabase.from('sbu').insert(chunk);
                if (error) throw error;
            }
    ...
    const handleClearData = async () => {
        if (!confirm('PERINGATAN: Apakah Anda yakin ingin menghapus SEMUA data SBU? Tindakan ini tidak dapat dibatalkan.')) return;

        setIsLoading(true);
        try {
            const { error: deleteError } = await supabase.from('sbu').delete().gt('id', 0);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const { error } = await supabase.from('sbu').delete().eq('id', id);
            if (error) throw error;
            fetchData();
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-navy-900 tracking-tight">Standar Biaya Umum (SBU)</h1>
                    <p className="text-gray-500 text-sm">Referensi Standar Biaya Umum</p>
                </div>
            ...
            <Card>
                <CardHeader className="py-4 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
                    <CardTitle className="text-base font-medium">Data SBU <span className="text-xs text-gray-400 font-normal ml-2">(Total: {totalItems})</span></CardTitle>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden">
                        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="text-lg font-bold text-gray-900">{isEditing ? 'Edit Item SBU' : 'Tambah Item SBU'}</h3>
